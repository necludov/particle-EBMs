...
if __name__ == '__main__':
    ...
    parser.add_argument('--ld_sigma', type=float, default=np.sqrt(1e0))
    parser.add_argument('--ld_n_iter', type=int, default=30)
    ...
    net = networks.MNISTSmallConvNet(nc=64, relu=args.relu)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == 99:
            ...
            torch.save({'args': args,
                        'ebm_state_dict': ebm.state_dict()}, 
                       './checkpoints/' + logger.name + '_100epochs')
            torch.save(particles, './checkpoints/particles_' + logger.name + '_100epochs')
    logger.save()
